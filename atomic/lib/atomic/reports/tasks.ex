defmodule Atomic.Reports.Tasks do
  ...
  @impl true
  def prepare_data(%{user_id: user_id, period: :week}) do
    now = Timex.now()

    start_of_week = Timex.beginning_of_week(now)
    end_of_week = Timex.end_of_week(now)

    {:ok, fetch_period(user_id, start_of_week, end_of_week)}
  end

  def prepare_data(%{user_id: user_id, period: :month}) do
    now = Timex.now()

    start_of_month = Timex.beginning_of_month(now)
    end_of_month = Timex.end_of_month(now)

    {:ok, fetch_period(user_id, start_of_month, end_of_month)}
  end
  ...
  def prepare_data(%{
    ...
    {:ok, fetch_period(user_id, start_time, end_time)}
  end
